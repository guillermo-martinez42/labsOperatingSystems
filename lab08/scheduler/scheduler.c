...
void fifo_schedule(Thread *thread)
{
    ...
    // Initialize base_time on first call
    if (base_time == 0)
    {
        base_time = thread->start_time - thread->arrival_time;
    }
    
    double waiting_time = thread->start_time - (base_time + thread->arrival_time);
    ...
    thread->turnaround_time = thread->completion_time - (base_time + thread->arrival_time);
